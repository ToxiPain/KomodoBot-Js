...
export function LogMessage(m, sock) {
    ...
    try {
        ...
        else if (m.message.documentMessage) content = `[Documento: ${m.message.documentMessage.fileName || 'sin nombre'}]`
        else if (m.message.audioMessage) content = m.message.audioMessage.ptt ? '[PTT]' : '[Audio]'
        else if (m.message.contactMessage) content = `[Contacto: ${m.message.contactMessage.displayName || 'sin nombre'}]`
        else if (m.message.contactsArrayMessage) content = '[Lista de contactos]'
